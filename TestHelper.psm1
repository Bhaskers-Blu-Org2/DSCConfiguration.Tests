...
function Wait-ConfigurationCompilation
{
    ...
    try 
    {
        while (@('Published','Suspended') -notcontains (Get-AzureRmAutomationDscCompilationJob -ResourceGroupName $ResourceGroupName `
...
function Wait-NodeCompliance
{
    ...
    try 
    {
        ...
        foreach ($Node in $Nodes) {
            while (@($null, 'InProgress') -contains (Get-AzureRMAutomationDSCNodeReport -ResourceGroupName $ResourceGroupName `
            -AutomationAccountName $AutomationAccountName -NodeID $Node.ID)) {
                Start-Sleep -Seconds 15
            }
        }
    }
